...
from draw import draw_letter, draw_like, draw_two_multi_pron, draw_letter_space, draw_new_line
...
for index in all_words.keys():
    ...
    for word in all_words[index]:
        ...
        draw_letter(paragraph1, word + " ", True)
    ...
    for x in range(min(6, len(selected_letters))):
        draw_letter_space(paragraph2, selected_letters[x])
        if x % 2 == 1 and x != 5:
